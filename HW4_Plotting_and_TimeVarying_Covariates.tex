...
\hlstd{data} \hlkwb{<-} \hlkwd{join}\hlstd{(data, plan,} \hlkwc{by} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"ID"}\hlstd{,} \hlstr{"wave"}\hlstd{))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}join"{}}}\begin{alltt}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in checkConv(attr(opt, "{}derivs"{}), opt\$par, ctrl = control\$checkConv, : Model failed to converge with max|grad| = 0.0396502 (tol = 0.002, component 1)}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'plans' not found}}\begin{alltt}
\hlkwd{summary}\hlstd{(modTV1)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(modTV1): object 'modTV1' not found}}\begin{alltt}
\hlstd{fixed.frame} \hlkwb{<-} \hlstd{data} \hlopt{%>%}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in mean(plans, na.rm = T): object 'plans' not found}}\begin{alltt}
\hlstd{fixed.frame} \hlkwb{<-}
  \hlkwd{data.frame}\hlstd{(}
    \hlkwd{expand.grid}\hlstd{(}
      \hlcom{# here, you add values for your time variable and predictors}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(modTV1, newdata = ., re.form = NA): object 'modTV1' not found}}\begin{alltt}
\hlstd{fixed.frame} \hlopt{%>%}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in factor(plans, levels = unique(plans), labels = c("{}-1SD"{}, "{}0SD"{}, : object 'plans' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(modTV2): object 'modTV2' not found}}\begin{alltt}
\hlcom{#interaction not significant}
\hlcom{#planning does not moderate the time course of weight loss}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(modTV2, newdata = ., re.form = NA): object 'modTV2' not found}}\begin{alltt}
\hlstd{fixed.frame} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{Plan} \hlstd{=} \hlkwd{factor}\hlstd{(plans,} \hlkwc{levels} \hlstd{=} \hlkwd{unique}\hlstd{(plans),} \hlkwc{labels} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"-1SD"}\hlstd{,} \hlstr{"0SD"}\hlstd{,} \hlstr{"1SD"}\hlstd{)))} \hlopt{%>%}
  \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= wave,} \hlkwc{y} \hlstd{= pred,} \hlkwc{color} \hlstd{= Plan))} \hlopt{+}
    \hlkwd{geom_line}\hlstd{(}\hlkwc{size} \hlstd{=} \hlnum{2}\hlstd{)} \hlopt{+}
